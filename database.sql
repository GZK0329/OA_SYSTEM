...
(
    `miaosha_id`  bigint(11) NOT NULL AUTO_INCREMENT COMMENT '秒杀Id 主键',
    `name`        varchar(255)                        NOT NULL COMMENT '秒杀商品名——会议室种类',
    `number`      int                                 NOT NULL COMMENT '库存数量',
    `create_time` timestamp DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT '创建时间',
    `start_time`  timestamp                           NOT NULL COMMENT '开始时间',
    `end_time`    timestamp                           NOT NULL COMMENT '结束时间',
    PRIMARY KEY (`miaosha_id`) USING BTREE,
...
VALUES ('小会议室', 10, 2021 - 09 - 10 09:00:00, 2021 - 09 - 10 10:00:00),
       ('中会议室', 5, 2021 - 09 - 10 10:00:00, 2021 - 09 - 10 11:00:00),
       ('大会议室', 3, 2021 - 09 - 10 11:00:00, 2021 - 09 - 10 12:00:00);



-- ----------------------------
-- Table structure for miaosha_result
-- ----------------------------

DROP TABLE IF EXISTS `miaosha_result`;
CREATE TABLE `miaosha_result`
(
    `miaosha_id`  bigint(11) NOT NULL AUTO_INCREMENT COMMENT '秒杀Id 主键',
    `open_id`     varchar(255) NOT NULL COMMENT '小程序中用户的openId 标识符 长时间授权符',
    `status`      int          NOT NULL DEFAULT -1 COMMENT '秒杀状态 -1失败 0成功 默认-1',
    `create_time` timestamp             DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT '创建时间',
    PRIMARY KEY (`miaosha_id`, `open_id`) USING BTREE,
    INDEX         idx_create_time(create_time)
) ENGINE = InnoDB CHARACTER SET = utf8;
